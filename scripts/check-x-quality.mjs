...
const server=createServer(async(req,res)=>{try{
 ...
 const file=path==='/admin/x'?'x-review.html':path.slice(1);if(!['x-review.html','x-review.js','instagram.css','feed.css','review-gallery.js','review-filters.js'].includes(file)){res.writeHead(404).end();return;}
...
await page.getByRole('button',{name:'숨기기',exact:true}).first().click();await page.waitForFunction(()=>document.querySelector('#message').textContent.includes('저장'));assert.equal(sqlite.prepare("SELECT COUNT(*) AS n FROM x_quality WHERE decision='hidden'").get().n,1);await page.locator('[data-status=hidden]').click();await page.waitForFunction(()=>document.querySelector('[data-status=hidden]').getAttribute('aria-pressed')==='true');assert.ok(await page.getByText('직접 숨긴 글',{exact:true}).count());await page.locator('[data-status=all]').click();await page.waitForFunction(()=>document.querySelector('[data-status=all]').getAttribute('aria-pressed')==='true');assert.equal(await page.locator('.review-card').count(),25);await page.locator('#next').click();await page.waitForFunction(()=>document.querySelector('#page').textContent.startsWith('2페이지'));await page.locator('[data-status=visible]').click();await page.waitForFunction(()=>document.querySelector('[data-status=visible]').getAttribute('aria-pressed')==='true');assert.ok((await page.locator('#page').textContent()).startsWith('1페이지'));assert.deepEqual(errors,[]);console.log('PASS: supplied 4 duplicate posts => 1, 3 unwanted examples hidden, missing example hidden, UI save and 1440/390px.');
