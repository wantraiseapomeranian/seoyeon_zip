...
const server=createServer(async(req,res)=>{
  ...
  try{
    ...
    if(!['instagram.html','instagram.js','instagram.css','feed.css'].includes(file)){res.writeHead(404).end();return;}
...
try{
  ...
  await page.click('#open-import');await page.fill('#json',JSON.stringify([{shortCode:'Example_123',ownerUsername:'sample_author',caption:'윤서연 tripleS · 미리보기 검증용 예시\n<script>alert(1)</script>',timestamp:'2026-09-10T00:00:00Z'}]));await page.click('#import-submit');await page.waitForSelector('.review-card');
  assert.equal(await page.locator('.review-caption script').count(),0);
