...
async function confirmDecision(page){await page.locator('.decision-dialog[open]').waitFor();const saved=page.waitForResponse(r=>r.request().method()==='POST'&&r.url().includes('/api/admin/'));await page.getByRole('button',{name:'판단 저장',exact:true}).click();assert.equal((await saved).status(),200);await page.locator('.decision-dialog').waitFor({state:'detached'});}
...
try{
  ...
  await page.click('#open-import');await page.fill('#json',JSON.stringify([{shortCode:'Example_123',ownerUsername:'sample_author',caption:'윤서연 tripleS · 미리보기 검증용 예시\n<script>alert(1)</script>',timestamp:'2026-09-10T00:00:00Z',childPosts:[{type:'Image',displayUrl:'https://scontent.cdninstagram.com/one.jpg'},{type:'Image',displayUrl:'https://scontent.cdninstagram.com/two.jpg'}]}]));await page.click('#import-submit');await page.waitForSelector('.review-card');
  ...
  await page.getByRole('button',{name:'피드에 표시',exact:true}).click();await confirmDecision(page);await page.waitForSelector('#empty:not([hidden])');const feed=await browser.newPage();await feed.goto('http://127.0.0.1:4178/feed.html?data=live&source=instagram');await feed.waitForSelector('.card');assert.equal(await feed.locator('.card').count(),1);assert.equal(await feed.locator('.category').first().textContent(),'Instagram');await feed.getByRole('button',{name:'피드 사진 다음',exact:true}).click();assert.equal(await feed.locator('.photo-count').textContent(),'2 / 2');await feed.close();await page.click('[data-status=kept]');await page.waitForSelector('.review-card');
  await page.reload();await page.click('[data-status=kept]');await page.waitForSelector('.review-card');
  await page.getByRole('button',{name:'보류',exact:true}).click();await confirmDecision(page);await page.waitForSelector('#empty:not([hidden])');await page.click('[data-status=held]');await page.waitForSelector('.review-card');
  assert.deepEqual(sqlite.prepare('SELECT action FROM review_audit_log ORDER BY rowid').all().map(r=>r.action),['KEEP','HOLD']);assert.deepEqual(errors,[]);assert.equal(sqlite.prepare('SELECT status FROM instagram_review').get().status,'held');
