...
const server=createServer((req,res)=>{const file=req.url.slice(1);if(!/^[\w.-]+\.(html|css|js)$/.test(file)){res.writeHead(404).end();return;}try{let body=readFileSync('validation/'+file,'utf8');if(file.endsWith('.html'))body=body.replace(/<script\b[^>]*>[\s\S]*?<\/script>/g,t=>t.includes('/date-controls.js')||t.includes('/theme.js')?t:'');res.setHeader('Content-Type',file.endsWith('.css')?'text/css':file.endsWith('.js')?'text/javascript':'text/html');res.end(body);}catch{res.writeHead(404).end();}});
...
try{
 ...
 for(const theme of ['light','dark'])for(const file of ['feed.html','x-review.html','instagram.html','review-history.html']){
  ...
  await page.getByRole('button',{name:'화면 설정',exact:true}).click();await page.getByRole('radio',{name:theme==='dark'?'어둡게':'밝게',exact:true}).check();await page.keyboard.press('Escape');
