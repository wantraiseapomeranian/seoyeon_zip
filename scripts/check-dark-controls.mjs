...
const server=createServer((req,res)=>{const file=req.url.slice(1);if(!/^[\w.-]+\.(html|css|js)$/.test(file)){res.writeHead(404).end();return;}try{let body=readFileSync('validation/'+file,'utf8');if(file.endsWith('.html'))body=body.replace(/<script\b[^>]*>[\s\S]*?<\/script>/g,'');res.setHeader('Content-Type',file.endsWith('.css')?'text/css':file.endsWith('.js')?'text/javascript':'text/html');res.end(body);}catch{res.writeHead(404).end();}});
await new Promise(r=>server.listen(0,'127.0.0.1',r));let browser;
try{
 browser=await chromium.launch({channel:'chrome',headless:true});const page=await browser.newPage();mkdirSync('.local/dark-controls',{recursive:true});
 ...
 console.log('PASS: light/dark date values and native scheme, SVG arrows, navigation and 390px overflow on feed/X/Instagram/history. Samsung forced-dark requires device verification.');
