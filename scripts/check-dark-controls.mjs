...
try{
 ...
 for(const theme of ['light','dark'])for(const file of ['feed.html','x-review.html','instagram.html','review-history.html']){
  ...
  const date=page.locator('input[type=date]').first();if(await date.count()){
   ...
   assert.equal(colors.value,'2026-09-18');assert.equal(colors.scheme,'light',`${file}: date stays light under ${theme} preference`);assert.equal(colors.bg,'rgb(255, 255, 255)');assert.equal(colors.color,'rgb(32, 37, 43)');assert.equal(colors.fill,colors.color);
   const visible=await date.evaluate(e=>{const s=getComputedStyle(e.parentElement.querySelector('.date-value'));return {bg:s.backgroundColor,color:s.color};});assert.deepEqual(visible,{bg:'rgb(255, 255, 255)',color:'rgb(32, 37, 43)'});
  }
