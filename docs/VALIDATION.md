...
## 새로고침 아이콘·클릭 영역 정렬 — 2026-09-11
- 변경 전 버튼 상자의 오른쪽만 콘텐츠 끝에 맞고 아이콘은 약13.84px 안쪽이었다.
- node scripts/check-review-refresh.mjs 통과: 세 페이지320/390/768/1280px의 실제 SVG path+stroke 끝,44px 클릭 영역·22px SVG 중심 일치,elementFromPoint 실제 클릭 대상,포커스 링,화면 경계·가로 넘침을 검증했다.
- PC/모바일 캡처 확인. 모바일은 전체 버튼을 화면 안에 유지해 아이콘이 정렬선보다 약1.84px 안쪽이다. CSS 외 앱 동작·서버·DB 변경 없음. 운영 배포는 최종 응답에 기록한다.

## 검토 내역 아이콘·헤더 정렬 확인 — 2026-09-11
- node scripts/check-review-refresh.mjs 통과. 세 페이지320/390/768/1280px에서 메뉴3개·현재 페이지1개·올바른 aria-current 대상,헤더/새로고침의 실제 path+stroke 오른쪽 끝,44px 클릭 영역과SVG 중심·실제 hit target·화면 경계를 확인했다.
- PC 헤더 정렬 오차0.1px 미만,모바일1px 미만. 모바일 헤더 클릭 영역은 화면 내부이며 포커스 링은 안쪽 표시. 검토 내역 모바일 캡처에서 내역 아이콘의 선택 상태 확인. 운영 배포 결과는 최종 응답에 기록한다.

## 검토 내역 썸네일 — 2026-09-11
- npm test 141/141 통과. 저장된 사진 주소 우선,기존 X/Instagram 게시물의 현재 미리보기 조회,게시물 삭제 시 미리보기 없음,감사 기록 무변경,서버 사진 주소 기록 및 클라이언트 주소 무시 검사 포함.
- node scripts/check-review-audit.mjs 통과: 시각 아래 썸네일 배치,lazy loading,원문 아이콘 접근성 이름,안전한 URL 및 기존 검토/재시도/필터/상세/포커스 동작. 모의 사진으로 PC/모바일 목록 배치 확인.
- 실제 사진 바이너리는 저장하지 않으며 URL 만료/삭제 시 대체 안내를 표시한다. DB 스키마 변경 없음. 운영 확인은 배포 후 최종 응답에 기록한다.

## 2026-09-11 검토 내역 목록 아이콘 정렬 검증
...
## 2026-09-14 maskable 검증
- 기존 PNG를 디코딩하여 512x512 및 불투명 배경 확인. 배경과 구별되는 로고 픽셀이 중심 반지름 177.40px 이내로, 표준 안전 영역(204.8px)을 충족.
- 이미지/공개 경로 변경 없이 manifest에 전용 maskable 항목 추가. 일반 아이콘과 분리된 purpose로 제공.
- Android 실기기 설치 및 런처 표시 미검증. 운영 manifest 응답 검증은 배포 후 최종 응답에 보고.

## 2026-09-14 X 검토함 조회 성능 검증
- 전체 `npm test` 144/144 통과. 기존 방식과 인덱스 결과 대조(순서/중복/역방향 near/제외), 동일 그룹 6000장 분류시 전수 쌍 생성 방지 확인.
- SQLite 3100건 통합 확인: visible 3098/pending 2, 첫 페이지 25건, 마지막 페이지 23건. author 필터 밖 유사 대상 보존 및 페이지 밖 pending 집계 확인.
- 가상 DB 응답으로 동일 API 비교: 사진 3000장 CPU 약 14485ms → 15ms; 6000장 47ms. DB 지연 없는 로컬 Node 측정이며 Cloudflare 실행 시간으로 간주하지 않음.
- 독립 읽기 전용 코드 리뷰 APPROVED(Critical/Major 없음). 운영 판정 데이터 수정 없이 배포 후 조회만 검증 예정.
- `npx wrangler deploy --dry-run` 통과(199.55 KiB, gzip 49.41 KiB). 코드 커밋 `8325197` Cloudflare 자동 배포 성공.
- 운영 Chrome 인증 세션에서 X 검토함 조회 확인: 전체 3008 / 표시 중 2945 / 숨김 63 / 검토 필요 0. 표시 중 1페이지와 다음 2페이지 로드 성공, 기존 1102 재현 안 됨. 실제 판정/DB 쓰기 없이 조회만 수행.
## 2026-09-15 URL 등록 사진·썸네일 — 로컬 검증

- X/Instagram URL 한 입력창, 별도 공급자 조회, D1 작업 상태/잠금/실행 번호, 기존 직접 등록 자료 보완과 관리자 재시도를 구현.
- 전체 Node 테스트 154/154 통과. 신규 10건은 실제 작성자·미디어 순서·실패 보존·재등록·Instagram 실행 재개/새로고침·응답 ID/CDN 검증·잠금 유실·검토 제외 유지·관리자 조회 권한을 확인.
- Playwright 390px/1280px: URL 등록→비동기 상태→직접 등록 갤러리 2장 넘기기→조회 실패 후 기존 미디어 유지. 가로 넘침 및 pageerror 없음. 기존 자료 관리 스모크도 통과.
- workerd/Miniflare+D1: 모든 마이그레이션(감사 로그 트리거 포함), 요청 waitUntil, Instagram 실행 번호 저장 및 scheduled 재개, 피드 이미지 연결 통과. 공급자는 로컬 응답 대역이며 실제 Apify 성공을 의미하지 않음.
- Wrangler dry-run 통과: 211.06 KiB / gzip 51.93 KiB. Windows 샌드박스의 esbuild 상위 디렉터리 접근 제한으로 같은 프로젝트에서 권한 확장 후 실행.
- 운영 기존 직접 등록 X 1건을 읽기 전용 조회: 실제 영상 썸네일 1개, 작성자/게시일 반환 확인. 미디어 원본은 저장하지 않음.
- 코드 리뷰의 잠금 유실 후 Apify 중복 시작 가능성을 회귀 테스트로 재현 후 수정. 재검토에서 추가 Critical/Major 없음.
- Instagram 개별 URL의 실제 Apify 실행은 미확인. 운영 DB에는 직접 등록한 Instagram 자료가 없어 임의의 새 게시물을 추가하지 않았다.
- 운영 0020 마이그레이션 적용 완료: 기존 직접 등록 1건 보존 및 보완 작업 생성. 과거 0012–0017 일부의 적용 이력 누락이 있어 해당 파일들은 재실행하지 않고 이번 파일만 지정 적용했다.

### 운영 배포 확인 — 2026-09-15

- 기능 커밋 `5510620` main 푸시 및 GitHub Workers Builds success. 활성 Worker 버전 `af3c913a-b6aa-4ef2-bf79-c60c859bebb3` 100% (2026-09-15 01:55 UTC). 직접 Wrangler 코드 배포는 하지 않았다.
- 기존 직접 등록 X 1건이 Cron에서 ready/error=null로 완료. 영상 썸네일 1개 및 실제 작성자·게시일 보완. `/api/feed?source=manual` 200/1건, Chrome 실제 이미지 표시 확인.
- 비로그인 `/api/manual-posts` 401 확인. 기존 관리자 인증을 유지하며 브라우저 세션이 만료된 경우 관리자 재로그인이 필요하다.
- 신규 관리 UI는 로컬 소유자 세션으로 검증했고, 운영 브라우저에서는 공개 피드 썸네일을 확인했다. Instagram 실제 외부 실행은 위 미확인 항목을 유지한다.

## 2026-09-15 직접 등록 목록 접기·지연 조회

- 기본 접힘 details, 펼칠 때만 목록 조회, 5개씩 페이지 교체, 새 등록 시 첫 페이지 자동 펼침 적용.
- 접기·탭 이동·모달 닫기에서 AbortController와 응답 버전으로 진행 중인 조회 및 폴링을 중단. 모달 재열기 시 펼친 목록은 다시 조회. 서버 사진 수집은 영향 없음.
- 관련 Node 테스트 14/14 통과. 12건 표본을 5/5/2로 중복 없이 조회.
- Playwright: 최초 접힘에서 관리자 목록 요청 0건, 신규 등록 자동 펼침, 페이지 교체/마지막 페이지, 접힌 뒤 5.5초간 추가 요청 없음, 재펼침 조회, 390px 가로 넘침 없음. 기존 갤러리/실패 보존 흐름도 통과.
- Wrangler dry-run 통과 (211.06 KiB / gzip 51.93 KiB). DB 마이그레이션 없음. 코드 리뷰에서 Critical/Major 없음.

## 2026-09-15 상태 새로고침 아이콘
- 자료 관리의 텍스트 버튼을 기존 새로고침 SVG와 modal-icon 스타일로 교체. aria-label/title 유지.
- 기존 Playwright 직접 등록 흐름 통과: 상태 새로고침 클릭, 재시도, 5건 페이징, 접기/펼치기 및 가로 넘침 없음. HTML 외 동작 코드 변경 없음.

## 2026-09-15 사진 새로고침 아이콘
- ready 항목에 기존 새로고침 SVG와 44px 아이콘 버튼 적용. aria-label/title 및 재시도 대기 안내 유지.
- 기존 Playwright 직접 등록 흐름 통과: 사진 새로고침 접근성 이름으로 클릭, 실패 시 기존 사진 보존, 페이징 및 접기/펼치기. 390px 화면 캡처에서 아이콘 표시와 가로 넘침 없음 확인.

## 2026-09-15 운영 관리 1단계

- 신규 운영 API/관리자 페이지 및 자료 관리 진입 링크. GET 외 405, DB 실패 503, private no-store. 운영/공급자 쓰기 없음.
- 전체 Node 테스트 165/165 통과. 신규 10건: 실제 SQLite 상태 집계, 읽기 전용(total_changes 불변), 중지/완료/재시도/잠금/지연, Instagram 현재 작업 분리, 비밀값 제외, 저장 건수, X 16계정 순차 처리 대기, JWT/쿠키 소유자 접근 및 익명·잘못된 토큰의 API/모든 자산 차단.
- Playwright check-operations: 실제 운영 API+SQLite 연결, 320/390/768/1280px 가로 넘침 없음, 계정 목록 접기, 새로고침 중복 차단, 실패 후 이전 기록 유지/복구, 인증 만료 시 내용 제거, 자료/수집 관리 이동, 방문자 링크 숨김, 쓰기 요청 0건. 모바일/PC 캡처 시각 확인.
- 기존 check-public-admin 통과: 방문자/관리자 구분, 세션 실패·만료·반응형 회귀.
- 코드 읽기 전용 검토에서 Critical/Major 없음.
...
- 운영 배포: a70f2ba GitHub Workers Builds completed/success 확인. 배포 후 루트는 /feed로 이동하여200, 비로그인 /api/admin/x는401로 보호 유지. 실제 운영 소유자 세션의 브라우저 조회는 미검증이며 위 운영 D1 직접 대조 및 로컬 브라우저 검증과 구분한다.

## 2026-09-15 운영 관리 3단계 — 자료 증가 추이

- 신규 일별 집계·대표 조회 측정·최근30일 API/화면. 최초 저장값 유지, 저장 행은 본문/작성자/사진 주소/공급자 응답 없이 집계만 포함. 기존 소유자 인증 경계 재사용.
- TDD: 백엔드5건과 API/Cron 통합3건의 미구현 실패 확인 후 통과. KST 날짜 경계, 30일 범위, 누락일 delta=null, 범위 밖 전일 비교, 동시 중복, 저장 오류 후 재시도, 조회 실패와 null 측정값, D1 metadata 우선순위 및 UTF-8 응답 크기 확인.
- 최종 전체 테스트179/179 통과. 기존 X 검토함288조합 동등성/800개 동일 그룹/3000개 페이지 회귀 포함. 읽기 전용 코드 리뷰 APPROVED(Critical/Major 없음).
- check-operations: 실제 SQLite+운영 API 연결, 최초 기록 대기·전일 증감·비교일 누락·대표 조회 실패·이력 테이블 장애에서도 현재 운영 현황 유지·표 접기, 320/390/768/1280px 가로 넘침 없음. 기존 새로고침 중복 차단/실패복구/인증 만료/방문자 숨김 검증 유지. 외부 쓰기 요청0, 브라우저 오류0. 390/1280px 신규 구역 캡처 시각 확인. Impeccable detector [] 확인.
- check-operations-runtime --local: 실제 workerd/D1 신규 Cron 분기 실행, 수집 활성 설정에서도 외부 호출0,100개 표본 원본 유지, 두 번째 호출 skipped 및 집계1행 유지. 실측 DB크기303104byte/SQL6ms/읽기1087행/응답8437byte. 단일 로컬 표본이며 운영 성능으로 일반화하지 않음.
...
- 운영 화면 안의 알림함 구현. 외부 채널 답변 미수신으로 외부 전달은 미구현이며 외부로 전송하지 않음. 알림 데이터는 고정 항목 키/허용 레이블과 발생·복구·중지 시각뿐이다.
- 엔진15+통합5 신규 테스트. 일시 실패/15분 지속/최소5분 정상 재확인/재발/중지/unknown/조회 실패/점검 공백, 동시 실행·후속 실행의 lease 보호, batch 중간 만료 및 이벤트 INSERT 실패 시 원자적 rollback, 최근20건 순서·안전한 레이블·API 읽기 전용 검증. 전체199/199 통과.
- 리뷰에서 Instagram 처리 중 overdue 숨김으로 알림 연속성이 끊기는 문제, lease 만료 후 실패를 기록하지 않아 정상 확인이 이어지는 문제 지적. 실제 운영 상태 함수를 사용한 IG 통합 회귀와 자체 lease 만료 회귀 RED 확인 후 수정. pendingOverdue 추가·자체 만료 last_failure_at 기록, 후속 token 보호 유지. 재리뷰 APPROVED(Critical/Major 없음).
- 실제 workerd/D1: 일별/알림 Cron의 수집 분리, 동시 호출 발생1회, 재점검 중복 없음, CHECK fence 만료 시 상태/이벤트/성공시각 변경 없음 및 실패시각 기록, 이후 정상2회 복구 확인. 외부 호출0, 표본 게시물100건 유지. 15분·5분 관찰 간격은 로컬 합성 시각으로 검증했으며 운영 장애를 만들지 않음. 처음 추가한 만료 테스트 뒤 복구 검증은 과거로 돌린 표본 시각 때문에 실패했고, 정상 관찰을 시간순으로 진행하도록 검증기를 수정해 통과.
- check-operations 통과: 실제 DB/API 연결, 첫 확인 대기·활성 문제·20건 제한·복구/중지 문구·점검15분 지연·알림 저장소 장애에서 기존 현황 유지, 인증 만료 시 내용 제거. 320/390/768/1280px 문서 가로 넘침 없음,390px 알림 표 자체 가로 스크롤 없음. 최종 모바일 캡처 시각 확인, 반복 새로고침/이전 추이 기능 회귀 통과. Impeccable detector [] 확인 후 모바일 표 우선순위 보완.
...
## 2026-09-15 운영 현황 탭 정리 및 부분 실패 안내

- 요약 / 수집·사진 / 자료 증가 / 알림 4개 탭으로 재배치. 상태 판단 기준은 기본 접힘, 요약의 문제 링크는 해당 탭과 제목으로 이동한다. 한 번 받은 API 응답을 공유하며 탭 전환에 추가 조회·자동 폴링·쓰기 없음.
- TDD RED: 부분 실패 응답에서 상단 오류 안내가 비어 있음, ArrowRight로 탭이 이동하지 않음을 확인한 뒤 구현. 부분 실패는 role=alert에 실패 항목을 표시하고 전체 성공 안내를 제거한다.
- check-operations-tabs 통과: 탭 ARIA 연결/단일 패널, 방향키/Home/End/포커스, URL 해시·기존 제목 링크·새로고침 선택 유지, 부분 실패 각각/동시 및 복구, 503 이전 내용 유지, 401 내용 제거. 320/390/768/1280px 모든 탭 가로 넘침 없음, 쓰기/브라우저 오류0. 실제 공통 feed.css를 포함하며 배경·폰트·44px 새로고침 버튼도 확인. 최종 390px 요약 표본 높이863px, 모바일 캡처 시각 확인.
...
- 운영 확인: 4a16f98 Workers Builds completed/success. 인증된 Chrome 운영 화면에서 2026-09-17 10:30 KST 수집 오류·지연0개, 과거 범위 미확인16개(중지1개 포함), 요약 확인 필요 항목 없음 확인. 기존 지속 알림15개는 정상 연속 확인 대기 상태이며 이 시점 해제는 미확인.

## 2026-09-18 인스타 URL 갱신

- 운영 공개 인스타19개 게시물의 앞2장(1장이면1장) 요청: 11개 게시물에서 HTTP403 URL signature expired 확인. 단순 다음 버튼 오류가 아님. 원본/검토 상태 복구 전 스냅샷은 .local에만 저장.
...
## 2026-09-21 유튜브 분류 확장
- 새 분류 등록 테스트 invalid_input RED 확인 후 구현. npm test220개 통과. check-youtube: COSMO 라이브 등록→공식 콘텐츠 보류→기타 제외 및 기존 중복/감사/반응형 검증 통과.
- 0025 운영 적용 성공(12쿼리114.42ms). 전후 영상157/리비전합3/직접등록2/발견182/감사3 보존 확인, 운영 PRAGMA foreign_key_check 위반0건. 로컬에서는 기존 제외 판정·리비전·전체 영상 행·발견 경로 동일 및 FK 무결성, 피드 뷰 유지 확인.

## 2026-09-21 지연 개인 직캠 혼입 방지
- 신규 필터 테스트 RED 확인 후 구현. 전체223개 통과. 지서연 안의 서연, Ji SeoYeon 안의 SeoYeon을 긍정 단서로 보지 않으며 NFKC 정규화 적용. 공동 이름/정보 불명확은 보존. 필터 후 발견 FK·기존 kept/revision 보존·cursor 원자적 전진 확인.
- 검색어 변경0026은 query 조건부 갱신, 이전 page token/window/lease 초기화 및 revision 증가. 재적용 멱등성과 다른 소스·enabled 보존 검증. 운영 적용1쿼리40.81ms 성공.
- Wrangler dry-run267.79KiB/gzip66.12KiB 통과. UI 변경 없음. 이미 저장된 미검토/제외 자료는 재분류하거나 삭제하지 않는다. 운영의 장기 혼입률은 아직 측정하지 않음.

## 2026-09-21 유튜브 검토량 축소 — 로컬 검증
- 전체226개 테스트 통과. 필터 이름 경계·해시태그·60초 경계·등록/비활성 채널·기존 판정/커서 보존, 수집 기준 밖 판정과 되돌리기 확인.
- 브라우저: 선택 초기화·3건 중1건 revision 충돌 시2건만 제외하고 이력2건 저장, 기존 직접 등록/분류/피드/검토 내역과320/390/768/1280px 회귀 통과. 화면 검사 후 전체 선택 라벨의 공통 grid 상속을 flex로 보완.
- Worker dry-run268.74KiB/gzip66.40KiB. 실제 배포/기존 후보 정리는 아직 미실행.
- 운영 읽기 전용 사전 집계:200건(표시71/제외48/미검토81), 신규 기준 밖74건(60초 미만39, Shorts 명시8, 제목 확인 불가23, 일반 콘텐츠 미등록 채널4). 기존 등록 채널0.

### 운영 적용 결과
- 9b09bee GitHub Cloudflare 자동 배포 성공. 실제 소유자 검토함에서25+20+29개를 COLLECTION_SCOPE 사유로 제외, 처리 실패0.
- 최종200건:표시71/제외122/미검토7. 사전 선정74개만 revision+1/제외 변경, 나머지126개 판정·분류·형식·revision·manual 모두 동일. 원본 영상 행 삭제0, 각 변경에 감사 이력74개 일대일 확인.
- 비로그인 GET /api/youtube/review 및 /api/youtube/status 모두401. 기존 Access/공개 피드 범위와 비밀값 변경 없음.
- 한계: 제목 기반 선별이라 직캠 표기가 없는 공연 영상도 기준 밖으로 남을 수 있음. 등록 채널0이므로 일반 콘텐츠 자동 수집은 채널 등록 이후 가능. 제외 탭 재검토·직접 등록을 유지.

## 2026-09-21 유튜브 일괄 표시·UI
- check-youtube.mjs 통과:제외3건을 kept/fancam/regular로 복원, 분류 및 일반 영상 확인 전 실행 불가, 선택 초기화 시 확인 해제, unavailable 영상422 거부와 기존 held 유지. 기존 일괄 제외의 revision 충돌 보존과 직접 등록/탭/검토 내역 회귀 통과.
- 화면4너비에서 가로 넘침 없음, 공통 아이콘22px/버튼44px 유지. 선택 전/선택 후 모바일·데스크톱 이미지 검사. 썸네일 누락 공간·체크박스 색상 보완 후 재확인.
- Worker dry-run268.74KiB/gzip66.41KiB. 서버 권한·수집 로직·운영 판정 데이터 변경 없음. 운영 일괄 게시 테스트를 위해 실제 영상을 임의로 공개하지 않음.
- 운영:0cb17ba Cloudflare 자동 배포 성공. 실제 소유자 화면에서25개 선택 시 일괄 작업(피드에 표시/제외), 공통5분류, 미확인 상태 실행 비활성 확인. 실제 썸네일과 카드 배치 확인 후 선택 해제. 운영 판정 요청은 보내지 않았음.

## 2026-09-21 공식 음악방송 과거 직캠 보충 수집
- npm test:235/235 통과. 채널 API 해석/검색 제약, 대기량20건, 최신 우선, 검색 예산 예약, 페이지/기간 전진, 저장 실패 롤백, 기존 수동/사람 판정 유지, 종료/활성화 재실행 안전성을 검증.
- node scripts/check-youtube.mjs:실제 로컬 API 기반 등록·검토·일괄 표시·충돌·기록 및320/390/768/1280px 회귀 통과.
- Wrangler4.130.0 dry-run:270.47KiB/gzip66.86KiB 성공. 별도 읽기 전용 코드 리뷰 Critical/Major 없음.
...
- 최종8ce8e31 Workers Builds 성공 확인 후4개 보충 소스 재개. KBS 첫 페이지 cursor 유지, 모든 보충 소스 last_error_code 없음. 최종202개(표시78/제외123/미검토1), 이전200개 판정 필드 변경0. 동명이인1건 EXCLUDE/NOT_SEOYEON 감사 기록 확인. 후속 배포 후 추가 보충 페이지는6시간 간격 때문에 아직 실행되지 않았으며, 최종 이름 필터는 재현 회귀로 검증.


## 2026-09-22 UI 검수 스킬 도입
- 공식 skill-installer로 vercel-labs/agent-skills의 063bee94c3f4df8453406c830b0a7df0f2860278 버전 web-design-guidelines 설치 성공. 고정 원본 SKILL.md와 설치 파일의 문자열 완전 일치 확인.
- 설치 파일 SHA256: F4647CA866A3ACCF763777F83E7682954F0187CD6BEA7EEA0399796652414E8F. name/description/외부 규칙 URL 및 프로젝트 Git 제외 정책 확인.
- DESIGN.md와 PLAN.md에 적용 순서·범위·재설치 방법 기록. 이번 작업은 스킬/문서 도입으로 앱 코드 테스트·빌드·실제 화면 검수는 수행하지 않음. 자동 스킬 인식과 실제 UI 적용 결과는 다음 작업에서 확인해야 하며 접근성 검증 통과로 간주하지 않음.
- 다른 기능의 기존 미커밋 변경은 포함하지 않는다. 커밋 전 문서 diff·개인정보/비밀값·공백 오류를 확인하며, GitHub 푸시와 자동 배포 결과는 별도로 확인한다.

## 2026-09-22 iPhone Safari 날짜 입력 가독성

- 사용자 제공 X·인스타 검토함 캡처에서 게시일 입력칸이 어둡고 텍스트가 보이지 않는 현상 확인. Safari 자체 달력 팝업 캡처가 아니다.
...
- 2026-09-22 초기/사진 로딩 회귀: 운영390px에서 다음 사진 선택22초 후 busy=true,complete=false,naturalWidth444(이전 이미지),loading=lazy,pbs.twimg.com 요청 미완료 재현. 원격 CDN 요청 지연의 원인은 단정하지 않으며 기존 무기한 대기·수동 이동 lazy 경로를 보완했다.
- review-gallery.js: 사용자 사진 이동/재시도 eager, 명시 eager 요청15초 미완료와 HTTP오류 시 spinner 정리·사진 영역 재시도. 초기 native lazy 사진에는 제한 시간을 걸지 않는다. check-photo-loading.mjs RED(20초 후 spinner1)/GREEN; retry·404→다음 사진 복구·destroy·화면밖50000px에서20초대기후 src보존 및 스크롤 로드 통과.
- 초기 표시: parser-blocking theme.js를 독립 async UI로 전환,215byte 테마 초기화만 HTML에 포함. Worker CSP에 정확한 sha256 해시만 허용(unsafe-inline 없음),6개 HTML 해시 일치 검사. 테마 CSS는 feed.css에 합치고 기존 theme.css URL은 호환용 빈 응답 유지. check-theme-first-paint.mjs RED(theme.js 보류시 footer 미생성)/GREEN(동일 조건에서 saved dark 첫표시404ms, feed.js 실행, 임의 inline차단). 404ms는 로컬 조건의 기능 검증치이며 운영 성능 목표 달성 주장이 아니다.
- 운영 네트워크 비교 표본은 문서 TTFB3.5–4.6초와 다른 JS 요청까지 지연됐고 후속 표본 navigation timeout으로 중단됨. theme defer 실험 결과를 일반적인 개선율로 사용하지 않는다. 초기 CSS/JS 병목 수정은 통제된 회귀 검사로 검증했다.
...
## 2026-09-23 피드 조회 반복 계산 축소
- 운영 읽기 전용 측정: X 약3080건, 검토 all 읽기58610행/49~82ms. 대표 사진 그룹 집계 대안81247행/47~50ms로 개선 근거가 없어 미적용. 최근 저장 기록82~124ms로 지속적 악화 추세는 확인하지 못했다.
- 피드 media=image 전후 각3회 실제 D1 SELECT 비교: 수정 전 합산 SQL시간 786.9214/707.749/828.816ms, 수정 후442.6255/392.1537/444.3898ms. 중앙값 약787→443ms(44% 감소), 읽기404579→206445행(49% 감소), SELECT5→3개. SQL별 시간을 합산한 수치이며 HTTP·이미지 로딩 시간과 다르다. 전후 readFeed 응답 전체 deepEqual 통과(total1672, 첫48건).
- Node SQLite 성장 표본(중복2건당 대표1개): 3000건172→98ms,6000건347→195ms, 응답 전체 동일. Cloudflare 런타임 성능 수치로 간주하지 않는다.
- 새 회귀는 실제 뷰 본문 평가 횟수202회로 RED 확인 후 최대102회로 GREEN. 전체 Node255개 통과; 기존 필터·날짜·동일날짜 양방향 페이지·누락/중복·총건수·숨김/검토/원문없음 제외·Instagram/YouTube/수동 피드 회귀 포함.
