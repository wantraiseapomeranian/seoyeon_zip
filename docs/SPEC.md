...
사진/영상/유튜브 탭을 제공한다. 유튜브는 서연 개인 직캠·외부 출연 일반 영상만 관리자 확인 후 게시한다. Shorts·단체 무대·팬 편집본·재업로드는 제외한다. URL 직접 등록은 서버 조회 미리보기와 사용자 확인을 거치며 기존 판정을 우회하지 않는다. 자동 발견은 전부 pending, 영상 ID 단위 중복 제거, 판정과 감사 원자적 저장 및 revision 충돌 검사를 적용한다.

공식 YouTube Data API를 서버 키로 사용한다. 최대 50개 상세 조회와 2MiB 응답 제한, API 호출 예산, 소스 lease, 페이지 원자적 저장을 적용한다. 자동 수집과 직접 등록은 독립 플래그다. 메타데이터는 7일 주기 갱신, 30일 만료 시 제거하며 사용자 판정과 분리한다. MP4 저장과 자체 재생은 제공하지 않는다. 채널 ID/@핸들은 관리자가 추가하면 공식 API 확인 후 추적한다. 기존 media=all API는 호환성을 위해 유지하되 기본 화면은 사진이다.

세부 계약과 운영 순서: docs/superpowers/plans/2026-09-21-youtube.md.

### 2026-09-21 유튜브 분류 확장

사용자 선택에 따라 개인 직캠(fancam), 외부 출연(appearance), COSMO 라이브(cosmo_live), 공식 콘텐츠(official), 기타(other)를 직접 등록과 검토함에 동일하게 제공한다. 분류를 늘려도 일반 영상 확인·Shorts 제외·기존 판정 보존 기준은 유지한다. 분류는 관리자 판정이며 공식 여부를 자동 인증하지 않는다.

### 2026-09-21 유튜브 동명이름 혼입 방지
외부 출연 검색을 트리플에스 윤서연으로 좁힌다. 신규 자동 발견 중 제목에 지연/지서연/JiYeon/Ji SeoYeon과 직캠/fancam/focus 등 표기가 있고 윤서연·독립된 서연/SeoYeon 표기가 없으면 저장에서 제외한다. 이름이 불명확하거나 공동 출연인 영상은 검토 대상으로 유지한다. 기존 저장 영상과 수동 등록은 이 자동 필터로 판정을 변경하지 않는다.
