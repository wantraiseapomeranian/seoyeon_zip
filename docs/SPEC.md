...
작업 상태는 pending/starting/waiting/ready/no_media/failed/existing이며 관리자 API에만 노출한다. GET /api/manual-posts는 20건 단위 조회, POST는 등록 또는 명시적 retry다. 중복 등록은 실패 작업을 자동 재실행하지 않는다. 새 요청의 waitUntil과 기존 Cron은 같은 60초 잠금을 사용하고 데이터/완료 상태는 원자적으로 저장한다. Apify 시작 응답을 잃으면 자동 중복 실행을 차단하고 사용자 재시도로만 새 실행을 허용한다. 과거 URL 등록 건도 동일한 작업 큐로 보완한다.
