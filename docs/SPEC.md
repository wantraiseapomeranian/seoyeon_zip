...
- 약 30분 갱신, 날짜 제한 없이 초기 최대 20페이지 수집. 실시간 알림과 전체 과거 백필은 제외.
...
2026-09-09 후속 구현: 소스별 최신 첫 페이지와 과거 cursor 이어받기를 번갈아 처리한다. 과거 보완 중 최신 조회는 약 60분, 보완 중지 후 약 30분이다. 처음 저장한 최신 페이지의 cursor로 과거 탐색을 시작하고 이후 최신 조회는 그 cursor를 덮어쓰지 않는다. 최신 조회도 날짜 제한 없이 판별한다. 첫 페이지를 넘어서는 급증분은 누락될 수 있어 전체 범위 완전성을 보장하지 않는다.
...
- 최초 cycle은 날짜 제한 없이 최대 20페이지를 탐색한다. 모든 과거 글을 보장하지 않는다.
...
## 2026-09-15 직접 URL 등록 이미지 보완

기존 V1 원문 링크 카드 제한을 확장한다. URL 입력은 하나를 유지하며 X·Instagram별로 사진/영상 썸네일과 작성자·게시일을 조회한다. 원본 미디어는 저장하지 않고 검증된 CDN URL만 저장한다. 소유자 등록 자료는 기존 직접 등록과 동일하게 피드에 표시하며, 자동 수집 자료와 중복되면 기존 검토 상태를 우선한다.

작업 상태는 pending/starting/waiting/ready/no_media/failed/existing이며 관리자 API에만 노출한다. GET /api/manual-posts는 5건 단위 조회, POST는 등록 또는 명시적 retry다. 중복 등록은 실패 작업을 자동 재실행하지 않는다. 새 요청의 waitUntil과 기존 Cron은 같은 60초 잠금을 사용하고 데이터/완료 상태는 원자적으로 저장한다. Apify 시작 응답을 잃으면 자동 중복 실행을 차단하고 사용자 재시도로만 새 실행을 허용한다. 과거 URL 등록 건도 동일한 작업 큐로 보완한다.

직접 등록 목록은 기본 접힘이며 펼칠 때 조회한다. 접기·탭 이동·모달 닫기는 상태 조회와 진행 중인 요청을 중단하며 수집 작업은 계속된다. 새 등록은 첫 페이지를 자동으로 펼친다. 이전·다음은 목록을 누적하지 않고 교체한다.
