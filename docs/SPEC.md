...
2. 진행 중 cycle의 cursor가 있으면 이어서 읽고, 아니면 첫 페이지를 읽는다.
...
- lastCompleteSyncAt: 이전 수집 경계까지 탐색을 마친 시각. UI 갱신 상태는 이것을 중심으로 보여준다.
...
연속 20페이지를 읽어도 경계에 도달하지 못하면 catchup 상태로 기록하고 운영 화면에 범위 미완료를 표시한다. 다른 소스가 굶지 않도록 1소스 1페이지씩 교대한다. API가 과거 범위를 더 주지 않으면 gap을 기록하며 전체 수집 성공으로 표시하지 않는다.
...
목록 수집 검증과 역할 검증을 분리한다. TRIPLES_FAN_FR는 일반 목록 수집 성공, 윤서연 COSMO 역할 검증 보류다. 이번 페이지에 역할 표본이 없다는 이유만으로 소스를 탈락시키지 않는다. 실제 작성자와 발견 소스는 독립적으로 저장한다. 상세 근거는 docs/VALIDATION.md를 따른다.
