CREATE VIEW instagram_photo_rows AS
 SELECT 'ig:'||p.code AS id,m.key AS position,m.value AS url,f.hash,p.data,p.imported_at,
 CASE WHEN f.hash IS NULL THEN 1 ELSE ROW_NUMBER() OVER(PARTITION BY f.hash ORDER BY p.code,CAST(m.key AS INTEGER)) END AS rank
...
CREATE VIEW instagram_feed_posts AS
 ...
 'media',json_group_array(json_object('kind','image','previewUrl',r.url))) AS data
