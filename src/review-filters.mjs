...
export function reviewFilters(params,{instagram=false}={}) {
 const day=params.get('date')||'';let range;
 if(day){try{range=publishedDayRange(day).map(Date.parse);}catch{throw Error('invalid_query');}}
 ...
 const matches=p=>{
  ...
  if(range){const time=Date.parse(date);if(!Number.isFinite(time)||time<range[0]||time>=range[1])return false;}
  else if(month&&(!date||new Date(new Date(date).getTime()+32400000).toISOString().slice(0,7)!==month))return false;
