...
export async function commitPage(DB,lease,page,nextState) {
  ...
  return commitState(DB,lease,nextState,[
    ...
    DB.prepare("DELETE FROM media WHERE post_id IN (SELECT json_extract(value,'$.id') FROM json_each(?))").bind(posts),
    ...
    DB.prepare("INSERT OR IGNORE INTO discoveries(post_id,source) SELECT json_extract(value,'$.id'),? FROM json_each(?)").bind(lease.source,posts)
  ]);
}
...
export async function scheduleRetry(DB,source) {
  const result=await DB.prepare(`UPDATE collection_state SET next_due_at=unixepoch()
    WHERE source=? AND enabled=1 AND catchup_status NOT IN ('needs_attention','gap')
...
export async function listSources(DB) {
  const {results}=await DB.prepare(`SELECT source,enabled,last_success_at,last_complete_sync_at,
    catchup_status,last_error_code FROM collection_state ORDER BY source`).all();
  return results;
}
