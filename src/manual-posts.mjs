...
export async function listManual(env,params){
 ...
 const {results}=await env.DB.prepare(`SELECT id,canonical_url,data,state,error,updated_at FROM (SELECT m.id,m.canonical_url,m.data,j.state,j.error,j.updated_at,m.created_at FROM manual_posts m JOIN manual_media_jobs j ON j.post_id=m.id UNION ALL SELECT 'yt:'||video_id,'https://www.youtube.com/watch?v='||video_id,json_object('platform','youtube'),'youtube_'||decision,NULL,metadata_fetched_at,created_at FROM youtube_videos WHERE manual=1) ORDER BY created_at DESC,id DESC LIMIT 6 OFFSET ?`).bind(offset).all();
