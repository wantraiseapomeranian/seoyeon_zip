...
const count=value=>value==null?'기록 없음':new Intl.NumberFormat('ko-KR').format(value)+'건';
...
function renderGrowth(history,generatedAt){
 ...
 values($('#growth-values'),[['X 저장 게시물',growthValue(latest.totals.x,latest.delta?.x,count)],['인스타 검토 자료',growthValue(latest.totals.instagram,latest.delta?.instagram,count)],['직접 등록 자료',growthValue(latest.totals.manual,latest.delta?.manual,count)],['유튜브 저장 영상',growthValue(latest.totals.youtube,latest.delta?.youtube,count)],['DB 크기',growthValue(latest.databaseBytes,latest.delta?.databaseBytes,bytes)]]);
 ...
 $('#growth-rows').replaceChildren(...items.map(item=>{
  ...
  for(const value of [count(item.totals.x),count(item.totals.instagram),count(item.totals.manual),count(item.totals.youtube),bytes(item.databaseBytes),query.status==='failed'?'측정 실패':metric(query.sqlMs,' ms'),metric(query.rowsRead,'행'),bytes(query.resultBytes)])row.append(node('td',value));
  return row;
 }));
}
function renderYouTube(yt){
 $('#youtube-status').replaceChildren(state(yt?.status??'unavailable'));$('#youtube-values').replaceChildren();$('#youtube-sources').replaceChildren();$('#youtube-error').textContent='';
 if(!yt?.counts){$('#youtube-error').textContent='유튜브 현황을 불러오지 못했어요. 새로고침으로 다시 확인해 주세요.';return;}
 values($('#youtube-values'),[['저장 영상',count(yt.counts.total)],['미검토',count(yt.counts.pending)],['표시 판정',count(yt.counts.kept)],['보류',count(yt.counts.held)],['제외',count(yt.counts.excluded)],['오늘 검색',yt.search.used+' / '+yt.search.limit+'회'],['마지막 수집 성공',timestamp(yt.lastSuccessAt)],['정보 갱신',timestamp(yt.refreshedAt)],['과거 보충 완료',yt.backfill.completed+' / '+yt.backfill.total+'채널']]);
 $('#youtube-error').textContent=needsAttention(yt.status)&&yt.error?'최근 오류 · '+yt.error:'';
 $('#youtube-sources').replaceChildren(...yt.sources.map(source=>{
  const li=node('li',undefined,'ops-source'),heading=node('div',undefined,'ops-source-heading');heading.append(node('strong',source.label),state(source.status));
  const dl=node('dl',undefined,'ops-values');values(dl,[['마지막 성공',timestamp(source.lastSuccessAt)],['다음 조회 가능',source.nextDueAt?timestamp(source.nextDueAt):['waiting','running'].includes(source.status)?'예약 실행에서 확인':statusNames[source.status]],['수집 범위',source.windowStart?source.windowStart.slice(0,10)+' ~ '+(source.windowEnd?.slice(0,10)??'현재'):'최근30일부터']]);li.append(heading,dl);
  if(source.error&&needsAttention(source.status))li.append(node('p','최근 오류 · '+source.error,'ops-warning'));return li;
 }));
}
function render(data){
 ...
 if(yt&&needsAttention(yt.status))issues.push({text:'YouTube · '+statusNames[yt.status],href:'/admin/youtube'});
 ...
 renderYouTube(yt);
 values($('#total-values'),[['X 저장 게시물',count(data.totals.x)],['인스타 검토 자료',count(data.totals.instagram)],['직접 등록 자료',count(data.totals.manual)],['유튜브 저장 영상',count(data.totals.youtube)]]);
