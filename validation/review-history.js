(() => {
  ...
  function originalIcon(url){const a=source(url);if(a.tagName!=='A')return a;a.className='review-icon history-original';a.setAttribute('aria-label','원문 보기');a.replaceChildren();const ns='http://www.w3.org/2000/svg',svg=document.createElementNS(ns,'svg'),path=document.createElementNS(ns,'path');svg.setAttribute('viewBox','0 0 24 24');svg.setAttribute('fill','none');svg.setAttribute('stroke','currentColor');svg.setAttribute('stroke-width','1.7');svg.setAttribute('stroke-linecap','round');svg.setAttribute('stroke-linejoin','round');svg.setAttribute('aria-hidden','true');path.setAttribute('d','M14 3h7v7M21 3 10 14M10 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-5');svg.append(path);const tip=node('span','원문 보기','icon-tooltip');tip.setAttribute('aria-hidden','true');a.append(svg,tip);return a;}
  function row(item){const li=node('li',null,'history-entry'),date=node('time',time(item.reviewedAt).replace(' (한국시간)','')),preview=node('div',null,'history-preview'),body=node('div',null,'history-entry-body'),controls=node('div',null,'history-entry-actions'),button=node('button','상세 보기');date.dateTime=item.reviewedAt;date.title='한국시간';preview.append(date,thumbnail(item.summary));body.append(node('h2',`${platforms[item.platform]||item.platform} · ${actions[item.action]||'검토 결정'}`),node('p',item.summary?.author?'@'+item.summary.author:'작성자 미상'),node('p',reason(item.reasonCode),'history-reason'));if(item.note)body.append(node('p',item.note,'history-note history-note-preview'));button.setAttribute('aria-label',`${actions[item.action]||'검토 결정'} 상세 보기 · ${time(item.reviewedAt)}`);button.onclick=()=>openDetail(item.id,button);controls.append(originalIcon(item.summary?.url),button);body.append(controls);li.append(preview,body);return li;}
